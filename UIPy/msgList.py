...
from PyQt5.QtGui import QFont, QColor, QPixmap, QMovie, QPainter, QPolygonF, QIcon, QCursor, QImage
from PyQt5.QtWidgets import QApplication, QLabel, QWidget, QListWidget, QListWidgetItem, QHBoxLayout, QSpacerItem, \
...
class TextItem(QWidget):
    ...
    def __init__(self, listItem, listView, text=DEFAULT_MSG, lr=True, device_name="MSE",ip = "127.0.0.1",parent_dialog=None):
    ...
    def mouseDoubleClickEvent(self, e):
        self.parent_dialog.change_other_ip_id_signal.emit(self.ip,self.device_name)
...
class BubbleImage(QLabel):
    ...
    def __init__(self, listItem, listView, img=DEFAULT_IMG, lr=True, maxWidth=50):  # 图片显示的最长宽度
        self.listItem = listItem
        self.listView = listView
        self.img = img
...
class ImageItem(QWidget):
    ...
    def __init__(self, listItem, listView, img=DEFAULT_MSG, lr=True, device_name=DEFAULT_HEAD,ip = "127.0.0.1",chat_dialog=None):
    ...
    def mouseDoubleClickEvent(self, e):
        self.chat_dialog.change_other_ip_id_signal.emit(self.ip,self.device_name)
...
class MsgList(QListWidget):
    ...
    def __init__(self,chat_dialog):
        ...
        # 设置所有样式锁定
        self.chat_dialog = chat_dialog # MsgList需要持有ｃｈａｔＤｉａｌｏｇ
        self.setStyleSheet(
    ...
    def addTextMsg(self, head,sz=DEFAULT_MSG, lr=True,ip = '127.0.0.1'):
        it = QListWidgetItem(self)
        wid = self.size().width()
        item = TextItem(it, self, sz, lr, head,ip,self.chat_dialog)  # 　让每一条都可以持有ｃｈａｔｄｉａｌｏｇ，让他们有更新ｃｈａｔｄｉａｌｏｇ的能力，
        # 增加必须指定本list和本item用于删除item
    ...
    def addImageMsg(self, head,img=DEFAULT_IMG, lr=True,ip="127.0.0.1"):
        it = QListWidgetItem(self)
        wid = self.size().width()
        item = ImageItem(it, self, img, lr, head,ip,self.chat_dialog)  # 增加必须指定本list和本item用于删除item
        # item.setEnabled(False) #对象灰度显示，不能导致ITEM不可选
        it.setSizeHint(item.sizeHint())
        self.addItem(it)
        self.setItemWidget(it, item)
...
if __name__ == '__main__':
    ...
    ml.addTextMsg('asdf',"Hello", True)
    ml.addTextMsg('asd',"World!", False)
    ml.addImageMsg('asd','ref/bq.gif', True)
    ml.addImageMsg('asd','ref/mt.gif', False)
    ml.show()
